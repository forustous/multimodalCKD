...
from libml.grad_cam import generate_grad_cam, getSmoothGradCAMPP, getGradCAMPP, getVanillaSaliencyMap, getSmoothVanillaSaliencyMap
...
class Fundus(models.MultiMoel):
    def train(self, dataset, batch, filters, repeat, scales, n_classes, test_case_name):
        ...
        # 2022_08_19_22_40_08_wd0.0001_B0_train_val_test_stratify
        ...
        # os.mkdir(os.path.join('./model', save_dir_name))
        ...
        if NUM_GPU == 1:
            ...
            def train_step(batch, model):
                with tf.GradientTape() as tape:
                    ...
                    # mixup_inputs = ratio * batch[0]['image'] + (1-ratio) * batch[0]['image2']
                    # mixup_labels = ratio * batch[1]['gt60'] + (1-ratio) * batch[1]['gt60_neg']
        ...
        #TODO: mutli-gpu setting
            ...
            def train_step(batch, model):
                def step_fn(inputs, model):
                    with tf.GradientTape() as tape:
                        # predictions = model([inputs[0]['image'], inputs[0]['age'], inputs[0]['sex']], training=True)# inputs[0]['blood'], inputs[0]['bilirubin'], inputs[0]['urobilinogen'], inputs[0]['ketone'], inputs[0]['protein'], inputs[0]['nitrite'], inputs[0]['glucose'], inputs[0]['leucocyte'], inputs[0]['ph'], inputs[0]['sg']])
                        predictions = model([inputs[0]['image'], inputs[0]['age'], inputs[0]['sex'], inputs[0]['blood'], inputs[0]['bilirubin'], inputs[0]['urobilinogen'], inputs[0]['ketone'], inputs[0]['protein'], inputs[0]['nitrite'], inputs[0]['glucose'], inputs[0]['leucocyte'], inputs[0]['ph'], inputs[0]['sg']], training=True)
                        # predictions = model([inputs[0]['image']], training=True)# inputs[0]['blood'], inputs[0]['bilirubin'], inputs[0]['urobilinogen'], inputs[0]['ketone'], inputs[0]['protein'], inputs[0]['nitrite'], inputs[0]['glucose'], inputs[0]['leucocyte'], inputs[0]['ph'], inputs[0]['sg']])
            ...
            def infer_step(batch, model):
                def step_fn(inputs, model):
                    # predictions = model([inputs[0]['image'], inputs[0]['age'], inputs[0]['sex']], training=False)#, batch[0]['blood'], batch[0]['bilirubin'], batch[0]['urobilinogen'], batch[0]['ketone'], batch[0]['protein'], batch[0]['nitrite'], batch[0]['glucose'], batch[0]['leucocyte'], batch[0]['ph'], batch[0]['sg']])
                    # predictions = model([inputs[0]['image']], training=False)#, batch[0]['blood'], batch[0]['bilirubin'], batch[0]['urobilinogen'], batch[0]['ketone'], batch[0]['protein'], batch[0]['nitrite'], batch[0]['glucose'], batch[0]['leucocyte'], batch[0]['ph'], batch[0]['sg']])
                    predictions = model([inputs[0]['image'], inputs[0]['age'], inputs[0]['sex'], inputs[0]['blood'], inputs[0]['bilirubin'], inputs[0]['urobilinogen'], inputs[0]['ketone'], inputs[0]['protein'], inputs[0]['nitrite'], inputs[0]['glucose'], inputs[0]['leucocyte'], inputs[0]['ph'], inputs[0]['sg']], training=True)
                    loss = bce(inputs[1]['gt60'], predictions['gt60'], sample_weight=inputs[2]['gt60'])
    ...
    def evaluate(self, dataset, filters, n_classes, load_dir_string, data_type_for_prediction_string, hospital_source_type_for_prediction_string):
        ...
        def inference_step(batch, model):
            # predictions = model([batch[0]['image'], batch[0]['age'], batch[0]['sex']], training=False)
    ...
    def export(self, dataset, batch, filters, repeat, scales, n_classes):
        ...
        # full_model = tf.function(lambda x, y, z:
        #                          (
        #                              model([x, y, z]),
        #                              model.optimizer.get_gradients(model([x, y, z])['image'], x))
        #                          )
        #
    ...
    def grad_cam(self, dataset, filters, n_classes, load_dir_string, data_type_for_prediction_string, hospital_source_type_for_prediction_string):
        ...
        save_dir_name = FLAGS.load_dir
        # print('save_dir_name',save_dir_name)
        # save_dir_name 2022_09_18_13_06_27_wd0.00003

        checkpoint_prefix = os.path.join(os.path.join('./model', save_dir_name), "best_checkpoint")
        # print('checkpoint_prefix',checkpoint_prefix)
        # ./model/2022_09_18_13_06_27_wd0.00003/best_checkpoint
        ...
        # /home/latte/project/retina_ckd/code/eGFR_age_sex_10ua_with_proteinuria_EfficientNet_SmoothGrad/libml/grad_cam.py
        # input_images, saliency_maps = getSmoothGradCAMPP(model, test_data, 'gt60', 'image')
        input_images, saliency_maps = getVanillaSaliencyMap(model, test_data, 'gt60', 'image')
        # input_images, saliency_maps = getSmoothVanillaSaliencyMap(model, test_data, 'gt60', 'image')
        # /home/latte/project/retina_ckd/code/eGFR_age_sex_10ua_with_proteinuria_EfficientNet_SmoothGrad/libml/grad_cam.py
        # input_images, saliency_maps = getGradCAMPP(model, test_data, 'gt60', 'image')
        # input_images, saliency_maps = generate_grad_cam(model, test_data, 'gt60', 'image')
        # print('input_images',len(input_images))
        # print('saliency_maps',len(saliency_maps))
        # input_images 10
        # saliency_maps 10
        # input_images (512, 512, 3)
        # saliency_maps (512, 512)
