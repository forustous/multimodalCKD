...
DATA_PATH   = '/home/latte/project/retina_ckd/data/raw_data/SCHPC'#'./data/images_one'
OUTPUT_PATH = './data/data_w_id_bright_haze995_standardization/'
CROP_PATH   = DATA_PATH + '_crop'
ODD_PATH    = DATA_PATH + '_odd'
HAZE_PATH = DATA_PATH + '_haze'
SIZE_PATH = DATA_PATH + '_size'
# NUM_WORKERS = 48
NUM_WORKERS = 86
...
haze_outlier_upper_bount=0.692 # 99.5%
...
class preprocess():
    ...
    def run(self, file_name):
        ...
        if self.image is not None:
            ...
            if gray.shape == (3456, 5184):
                x, y, z = 2620, 2620, 1640
            elif gray.shape == (3264, 4928):
                x, y, z = 2470, 2470, 1570
            elif gray.shape == (2448, 3696):
                x, y, z = 1840, 1840, 1180
            else:
                print(gray.shape, file_name)
                dp = 1.0
            ...
            if x == 0:
                crop_CN, cropped_mask = None, None
                cv.imwrite(os.path.join(SIZE_PATH, file_name), self.image)
            ...
            return crop_CN, cropped_mask, h, s, v

    def haze_scoring(self, crop) :
        crop_reitna = crop[83:429, 83:429].copy()
        src = cv.resize(crop_reitna, dsize=(512,512), interpolation=cv.INTER_AREA)
        src_gray = cv.cvtColor(src, cv.COLOR_BGR2GRAY)
        reverse = cv.bitwise_not(src_gray)
        low_kernel = np.ones((32,32), np.float32)/(32*32)
        low_img = cv.filter2D(reverse, -1, low_kernel)
        high_img = reverse - low_img
        ent_img = sfr.entropy(high_img, square(3))
        f = np.fft.fft2(ent_img)
        f_shift = np.fft.fftshift(f)
        abs_fouier_transform = np.abs(f_shift)
        power_spectrum = np.square(abs_fouier_transform)
    ...
    with tqdm(total=end-begin) as pbar:
        ...
        for file_name in image_list[begin:end]:
            ...
            if cropped_image is not None:
                ...
                eGFR         = int(this_df['eGFR_ckd'] < 60)
                eGFR45       = int(this_df['eGFR_ckd'] < 45)
                CKD          = int(this_df['eGFR_ckd'])

                num_eGFR += 1
                feat = {'image'         : _bytes_feature(cropped_image.reshape([-1]).tostring()),
                        'mask'          : _bytes_feature(cropped_mask.reshape([-1]).tostring()),
...
def main(argv):
    ...
    if os.path.exists(CROP_PATH): shutil.rmtree(CROP_PATH)
    ...
    np.random.seed(0)
    df = pd.read_csv('/home/latte/project/retina_ckd/data/table/severance/checkupcenter/SCHPC_eGFR_v7_delete_egfr_null_delete_one_image_data_delete_two_image_id_data_age2079.csv')

    dev_patients = df['patient_id'].unique()
    np.random.shuffle(dev_patients)

    processing(df, df['image_id'].unique(), 'SCHPC', chunk_size=(len(df) // NUM_WORKERS) + 1)
